...
public class StockManagerTest {
  ...
  public StockBook getMissedDefaultBook() {
    return new ImmutableStockBook(TEST_ISBN + 1, "Harry Potter and JUnit",
        "JK Unit", (float) 10, 0, 1, 0, 0, false);
  }

  public StockBook getSecondDefaultBook() {
    return new ImmutableStockBook(TEST_ISBN + 1, "Harry Potter and JUnit",
        "JK Unit", (float) 10, NUM_COPIES, 0, 0, 0, false);
  }
   ...
   */
  @Test
  public void testBooksInDemamd() throws BookStoreException {

    Set<StockBook> bookSet = new HashSet<>();
    bookSet.add(getSecondDefaultBook());
    storeManager.addBooks(bookSet);
    Set<BookCopy> bookToBuy = new HashSet<>();
    bookToBuy
        .add(new BookCopy(getSecondDefaultBook().getISBN(), NUM_COPIES + 1));
    try {
    ...
    assertTrue(booksInDemand.size() == 1
        && booksInDemand.contains(getMissedDefaultBook())
        && booksInDemand.get(0).getSaleMisses() == getMissedDefaultBook()
            .getSaleMisses());
    List<StockBook> booksInStorePostTest = storeManager.getBooks();
    // Check pre and post state are same
    assertTrue(booksInStorePreTest.containsAll(booksInStorePostTest)
  ...
  /**
   * Test behavior when the book store is empty
   * 
   * @throws BookStoreException
   */
  @Test
  public void testNoBooksInBookStore() throws BookStoreException {
    cleanupBooks();
    List<StockBook> booksInStorePreTest = storeManager.getBooks();
    List<StockBook> booksInDemand = null;
  ...
  public void testaddBookInvalidISBN() throws BookStoreException {
    ...
    booksToAdd.add(new ImmutableStockBook(TEST_ISBN + 1,
        "Harry Potter and Vivek", "JUnit Rowling", (float) 100, 5, 0, 0, 0,
        false)); // valid
  ...
  /**
   * Checks whether the insertion of a book with a negative number of copies is
   * rejected.
   */
  @Test
  public void testAddBookInvalidCopies() throws BookStoreException {
    ...
    booksToAdd.add(new ImmutableStockBook(TEST_ISBN + 2,
        "Harry Potter and Marcos", "JUnit Rowling", (float) 100, -1, 0, 0, 0,
        false)); // invalid copies
  ...
  public void testAddBookInvalidPrice() throws BookStoreException {
    ...
    booksToAdd.add(new ImmutableStockBook(TEST_ISBN + 1,
        ...
        false)); // valid
    booksToAdd.add(new ImmutableStockBook(TEST_ISBN + 2,
        "Harry Potter and Marcos", "JUnit Rowling", (float) -100, 5, 0, 0, 0,
        false)); // invalid price
  ...
  /**
   * Checks whether the insertion of a negative number of copies with addCopies
   * (N.B. not addBooks as above) is rejected
   */
  @Test
  ...
  /**
   * Checks whether the insertion of a number of copies for an invalid ISBN with
  ...
  /**
   * Checks whether the insertion of a number of copies with an ISBN not in the
   * system (N.B. not addBooks as above) is rejected
   */
  @Test
  ...
  public void testRemoveBooks() throws BookStoreException {
    ...
    StockBook book3 = new ImmutableStockBook(TEST_ISBN + 2,
        "The C Programming Language", "Dennis Ritchie and Brian Kerninghan",
        (float) 50, NUM_COPIES, 0, 0, 0, false);
